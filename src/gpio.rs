...
macro_rules! convert_pin {
    ($input:ty, $output:ty) => {
        impl<const P: char, const N: u8> From<crate::gpio::Pin<P, N, $input>>
            for crate::gpio::Pin<P, N, $output>
        {
            fn from(_: crate::gpio::Pin<P, N, $input>) -> Self {
                let gpio = unsafe { &(*crate::gpio::Gpio::<P>::ptr()) };
                unsafe {
                    // Set the mode in the register N*2 as there is two bits to configure
                    gpio.moder.modify(|r, w| {
...
macro_rules! from_pin {
    ($port_id:expr, $pin_number:expr, $afi:expr, $input:ty, $mode:ty) => {
        impl From<super::Pin<$port_id, $pin_number, $input>>
            for crate::gpio::Pin<$port_id, $pin_number, super::Alternate<$afi, $mode>>
        {
            fn from(_: super::Pin<$port_id, $pin_number, $input>) -> Self {
                let gpio = unsafe { &(*crate::gpio::Gpio::<$port_id>::ptr()) };
                unsafe {
                    // Set the mode in the register N*2 as there is two bits to configure
                    gpio.moder.modify(|r, w| {
                        w.bits(
                            (r.bits() & !(0b11 << $pin_number * 2))
                                | (<super::Alternate<$afi, $mode>>::MODE << $pin_number * 2),
                        )
                    });
                    // Write the config into the type register
                    gpio.otyper.modify(|r, w| {
                        w.bits(
                            (r.bits() & !(0b1 << $pin_number))
                                | (<super::Alternate<$afi, $mode>>::TYPE << $pin_number),
                        )
                    });
                    // Write the config into the pull-up/pull-down register
                    gpio.pupdr.modify(|r, w| {
                        w.bits(
                            (r.bits() & !(0b11 << $pin_number * 2))
                                | (<super::Alternate<$afi, $mode>>::PULL << $pin_number * 2),
                        )
                    });
                };
                Self {
...
macro_rules! gpio {
    ($GPIOX:ident, $gpiox:ident, $port_id:expr, $PXn:ident, [
        $($PXi:ident: ($pxi:ident, $pin_number:expr, [$($afi: expr,)+], [$($afi2: expr,)+] $(, $MODE:ty)?),)+
    ]) => {
        mod $gpiox {
             use crate::rcc::{Reset, Enable};
             use super::sealed::PinMode;
             pub struct Parts {
                    $(
                        /// Pin
                        pub $pxi: $PXi $(<$MODE>)?,
            ...
            $(
                ...
                $(
                    from_pin!($port_id, $pin_number, $afi, super::Analog, super::PushPull);
                    from_pin!($port_id, $pin_number, $afi, super::Analog, super::OpenDrain);
                    from_pin!($port_id, $pin_number, $afi, super::Input<super::Floating>, super::PushPull);
                    from_pin!($port_id, $pin_number, $afi, super::Input<super::Floating>, super::OpenDrain);
                    from_pin!($port_id, $pin_number, $afi, super::Input<super::PullUp>, super::PushPull);
                    from_pin!($port_id, $pin_number, $afi, super::Input<super::PullUp>, super::OpenDrain);
                    from_pin!($port_id, $pin_number, $afi, super::Input<super::PullDown>, super::PushPull);
                    from_pin!($port_id, $pin_number, $afi, super::Input<super::PullDown>, super::OpenDrain);
                    from_pin!($port_id, $pin_number, $afi, super::Output<super::PushPull>, super::PushPull);
                    from_pin!($port_id, $pin_number, $afi, super::Output<super::PushPull>, super::OpenDrain);
                    from_pin!($port_id, $pin_number, $afi, super::Output<super::OpenDrain>, super::PushPull);
                    from_pin!($port_id, $pin_number, $afi, super::Output<super::OpenDrain>, super::OpenDrain);
                    $(
                        from_pin!($port_id, $pin_number, $afi, super::Alternate<$afi2, super::OpenDrain>, super::PushPull);
                        from_pin!($port_id, $pin_number, $afi, super::Alternate<$afi2, super::PushPull>, super::OpenDrain);
                    )+
                )+
            )+
...
gpio!(GPIOA, gpioa, 'A', PAn, [
    PA0: (pa0, 0, [1,7,8,9,10,14,15,], [1,7,8,9,10,14,15,]),
    PA1: (pa1, 1, [0,1,7,9,15,], [0,1,7,9,15,]),
    PA2: (pa2, 2, [1,7,8,9,12,14,15,], [1,7,8,9,12,14,15,]),
    PA3: (pa3, 3, [1,3,7,9,12,13,15,], [1,3,7,9,12,13,15,]),
    PA4: (pa4, 4, [2,5,6,7,13,15,], [2,5,6,7,13,15,]),
    PA5: (pa5, 5, [1,2,5,14,15,], [1,2,5,14,15,]),
    PA6: (pa6, 6, [1,2,4,5,6,8,12,15,], [1,2,4,5,6,8,12,15,]),
    PA7: (pa7, 7, [1,2,4,5,6,8,14,15,], [1,2,4,5,6,8,14,15,]),
    PA8: (pa8, 8, [0,2,4,5,6,7,10,12,14,15,], [0,2,4,5,6,7,10,12,14,15,]),
    PA9: (pa9, 9, [2,4,5,6,7,9,10,14,15,], [2,4,5,6,7,9,10,14,15,]),
    PA10: (pa10, 10, [1,3,4,5,6,7,10,11,12,14,15,], [1,3,4,5,6,7,10,11,12,14,15,]),
    PA11: (pa11, 11, [5,6,7,8,9,10,11,12,15,], [5,6,7,8,9,10,11,12,15,]),
    PA12: (pa12, 12, [1,5,6,7,8,9,10,11,15,], [1,5,6,7,8,9,10,11,15,]),
    PA13: (pa13, 13, [0,1,4,5,7,10,13,15,], [0,1,4,5,7,10,13,15,], super::Debugger), // SWDIO, PullUp VeryHigh speed
    PA14: (pa14, 14, [0,1,4,5,6,7,13,15,],[0,1,4,5,6,7,13,15,], super::Debugger), // SWCLK, PullDown
    PA15: (pa15, 15, [0,1,2,4,5,6,7,8,9,14,15,],[0,1,2,4,5,6,7,8,9,14,15,], super::Debugger), // JTDI, PullUp
]);
